...
from fw_audit.executors.docker_executor import CONTAINER_WORKDIR, DockerExecutor, to_container_path
...
async def test_run_composes_docker_run_argv(monkeypatch, tmp_path: Path):
    ...
    async def fake_run_command(binary, args, *, settings=None):
        ...
        captured["args"] = args
        return CommandResult(command=[binary, *args], returncode=0, stdout="", stderr="", timed_out=False)
    ...
    argv = captured["args"]
    assert argv[:3] == ["run", "--rm", "--network=none"]
    assert "-v" in argv
...
async def test_run_forces_empty_command_prefix(monkeypatch):
    ...
    async def fake_run_command(binary, args, *, settings=None):
        ...
        return CommandResult(command=[binary], returncode=0, stdout="", stderr="", timed_out=False)
