...
**Current status:** Stages 1 and 2 (`fw_audit/stage1_ingestion/`, `fw_audit/stage2_extraction/`) are implemented. Stages 3–6 are empty placeholder packages (`fw_audit/stage3_rag/` … `fw_audit/stage6_reporting/`).
...
- **`normalize/`** — the Post-Decompilation Handler / C Normalizer. Sanitizes Ghidra's decompiled C (non-standard types, `CONCATxy`/`SUBxy`/`ZEXTxy`/`SEXTxy` intrinsics, illegal `::` switch labels, undeclared `in_*`/`unaff_*`/`extraout_*` register vars) into two targets: whole-program C for Joern (`JOERN_PIPELINE`) and per-function C for LLM analysis (`LLM_PIPELINE`). A generated prelude header (`prelude.py::PRELUDE_HEADER`) turns every non-standard *type* into a `typedef` and every intrinsic *macro* into a `#define` — zero rewriting risk; `passes.py` handles only what a declaration can't express (illegal tokens, undeclared identifiers, duplicate definitions), as pure `(str) -> str` functions run through `spans.py`'s tokenizer so they never touch a string/char literal or comment. `normalize(normalize(x)) == normalize(x)` is a hard invariant, tested directly.
