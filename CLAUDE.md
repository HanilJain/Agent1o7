...
**Current status:** Stages 1 and 2 (`fw_audit/stage1_ingestion/`, `fw_audit/stage2_extraction/`) are implemented. Stage 3 (`fw_audit/stage3_analysis/`) has ingest/whitelist (Step 1), tree-sitter function-only extraction (Step 2, `clean/`), and chunking (Step 3, `chunk/`) implemented; only queueing (Step 4, the `asyncio.Queue`/worker pool) remains. Stages 4–6 are empty placeholder packages (`fw_audit/stage4_analysis/` … `fw_audit/stage6_reporting/`).
...
# Run Stage 3 (consumes Stage 2's stage2_summary.json; ingest/whitelist always runs — clean/chunk are exercised via the debug flags below, queue is not yet implemented)
