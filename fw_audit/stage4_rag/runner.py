    ...
    *,
    ...
    if show_usage and registry is not None:
        summary_text = format_usage_summary(
            registry.snapshot(stage="4", run_id=args.run_id or "")
        )
        if summary_text:
            print(f"\n{summary_text}")
    return 0
...
def _cmd_debug(
    ...
    *,
    ...
    show_usage = args.usage if args.usage is not None else settings.llm_usage_console
    if show_usage and registry is not None:
        summary_text = format_usage_summary(registry.snapshot(stage="4-debug", run_id=""))
        if summary_text:
            print(f"\n{summary_text}")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    if settings.llm_usage_artifact and db_subfolder is not None:
        ...
        jsonl_path = usage_layout.usage_jsonl_path(usage_dir_, stage="4", run_id=run_id)

    log_ctx = (
        capture_run_log(settings, db_subfolder=db_subfolder, stage="4", run_id=run_id)
                ...
                if settings.llm_usage_artifact and db_subfolder is not None:
                    registry.write_report(
                        usage_layout.usage_report_path(
                            usage_layout.usage_dir(db_subfolder), stage="4", run_id=run_id
                        ),
                        stage="4",
