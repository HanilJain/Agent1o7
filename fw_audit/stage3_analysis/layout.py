...
Layout, under `<db_subfolder>/stage3/`::
    ...
    chunks/<chunk_id>.c       (--debug only; Step 4 — not written yet)
...
function's docstring) while `chunks/` will hold one file per Step 3 chunk once
chunking exists. Don't conflate them.
...
def chunks_dir(stage3_dir_: Path) -> Path:
    """Debug-only dump location for chunk text — reserved for Step 4;
    Component 1 never writes chunk payloads to disk. Unrelated to
    `debug_dir` above, which is Step 1's own per-target debug dump, not a
    per-chunk one."""
    return stage3_dir_ / "chunks"


def chunk_filename(chunk_id: str) -> str:
    """Reserved for Step 4. `chunk_id` is already filesystem-safe by
    construction (`<bin_id>#<ordinal>`, and `bin_id` is itself
    `_SAFE_NAME_RE`-sanitized by `stage2_extraction.layout.bin_id`), so this
    only needs to swap the `#` Step 3 uses as a human-readable separator for
    something that isn't awkward in a Windows filename."""
